    ...
    with open(config_file, 'r') as f:
        for line in f:
            ...
            if current_section == 'pdb_files':
                # 確保每個文件被添加到列表中
                if line not in config['input_files']:
                    config['input_files'].append(line)
                    print(f"Added input file: {line}")
...
def main():
    ...
    try:
        ...
        # Print input files for confirmation
        print(f"Found {len(config['input_files'])} input files to process:")
        for i, file_path in enumerate(config['input_files']):
            print(f"  {i+1}. {file_path}")
        
        if not config['input_files']:
            raise ValueError("No input files specified in config or none of the specified files exist")
        ...
        for i, input_file in enumerate(config['input_files']):
            ...
            # Add a short pause between processing files to avoid system overload
            time.sleep(1)
